...
public:
  ...
  Subdet detIdToSubDet(uint32_t const key) const;

  /// Resolves the number of elinks associated with the detector identified by the given raw DetId
  unsigned int detIdToNElinks(uint32_t const key) const;
  ...
  /// Returns true if the cabling map has a record corresponding to an entry for number of elinks
  bool knowsNElinks(unsigned int key) const; 
  ...
  void insert(DTCELinkId const&, uint32_t const, unsigned int const, unsigned int const, Subdet const, unsigned int const);
...
private:
  ...
  std::unordered_map<uint32_t, Subdet> cablingMapDetIdToSubDet_;
  std::unordered_map<uint32_t, unsigned int> cablingMapDetIdToNElinks_;
